...
def initialize_weights_kaimingnormal_forOC(*models):
    """
    Initialize Model Weights
    """
    for model in models:
...
def initialize_weights_kaimingnormal_forOC_whichOC(*models, whichoc=4):
    """
    Initialize Model Weights
    """
    for model in models:
        for module in model.modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                ...
                isinstance(module, nn.GroupNorm) or isinstance(module, nn.SyncBatchNorm):
                if(whichoc==1):
                    nn.init.normal_(module.weight, mean=0.0, std=0.25)
                elif(whichoc==2):
                    nn.init.normal_(module.weight, mean=0.0, std=0.50)
                elif(whichoc==3):
                    nn.init.normal_(module.weight, mean=0.0, std=0.75)
                elif(whichoc==4):
